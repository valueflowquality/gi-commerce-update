...
angular.module('gi.commerce').directive('giCheckout', [
  'giCart', 'usSpinnerService', 'Address', 'giPayment', '$modal', 'giUtil', '$q', '$timeout', 'deviceDetector', '$location', function(Cart, Spinner, Address, Payment, $modal, Util, $q, $timeout, deviceDetector, $location) {
    return {
      restrict: 'E',
      scope: {
        model: '='
      ...
      link: function($scope, element, attrs) {
        var cardElement, fieldUsed, invokeCartPayment, processItemDiscount, purchaseItems, setPaymentDate, stopSpinner, updateCartCountry, wrapSpinner;
        ...
        invokeCartPayment = function() {
          var deferred;
          deferred = $q.defer();
          $scope.inPayment = true;
          Cart.handleSubscriptionRequest().then(function() {
            $scope.inPayment = true;
        ...
        purchaseItems = function() {
          ...
          deferred = $q.defer();
          $scope.inPayment = true;
          Cart.handleItemPurchase().then(function() {
            $scope.inPayment = true;
            return deferred.resolve();
          }, function() {
            $scope.inPayment = false;
        ...
        setPaymentDate = function() {
          ...
          if ((ref = $scope.model) != null ? (ref1 = ref.me) != null ? (ref2 = ref1.user) != null ? ref2.trialUsed : void 0 : void 0 : void 0) {
            return $scope.nextPaymentDate.setFullYear($scope.nextPaymentDate.getFullYear() + 1);
          } else {
            return $scope.nextPaymentDate.setDate($scope.nextPaymentDate.getDate() + 30);
          }
        };
        ...
        $scope.$watch('model.me', function(me) {
          if ((me != null ? me.user : void 0) != null) {
            ...
            return $scope.isTrial = true;
          }
        });
        $scope.$watch('cartItems.length', function() {
          var i, item, itemData, itemDiscountFound, len, ref, subscriptionFound;
          ...
          if (!subscriptionFound) {
            $scope.subscriptionItem = void 0;
          }
          if (!itemDiscountFound) {
            $scope.itemDiscountInCart = false;
          }
          return processItemDiscount();
        });
        ...
        $scope.$watch('model.userCountry', function(newVal) {
          if (newVal != null) {
            return $scope.cart.billingAddress.country = newVal.code;
          }
        });
        $scope.$watch('cart.billingAddress.country', function(newVal) {
          if (newVal) {
            return wrapSpinner(updateCartCountry(newVal));
          }
        });
        updateCartCountry = function(country) {
          return Cart.setCountry(country).then(function() {
            if ($scope.checkoutForm.vat) {
              return $scope.checkoutForm.vat.$validate();
            }
          });
        };
        $scope.purchase = function(cart) {
          var i, item, len, ref, subscriptionFound;
          subscriptionFound = false;
          ref = $scope.cartItems;
          for (i = 0, len = ref.length; i < len; i++) {
            item = ref[i];
        ...
        $scope.$on('giCart:paymentFailed', function(e, data) {
          ...
          if (data != null) {
            ...
            if (data.message != null) {
              if (((ref = data.message.raw) != null ? ref.message : void 0) != null) {
                $scope.errorMessage = data.message.raw.message;
              } else {
                $scope.errorMessage = data.message;
              }
            }
            if (((ref1 = data.raw) != null ? ref1.message : void 0) != null) {
        ...
        $scope.showLoginForm = function() {
          ...
          if ($scope.isMobile()) {
            return $location.path('login').search('next', '/a/checkout');
          } else {
        ...
        $scope.closeModal = function(closefn) {
          return closefn();
        };
        ...
        $scope.$watch('checkoutForm.$pending', function(pending) {
          if (pending != null) {
            return $scope.cart.setCheckoutFormValidity(false);
          }
        });
        $scope.$watch('cart.business', function(business) {
          if (!business && $scope.cart.company) {
            $scope.cart.company.VAT = null;
            $scope.cart.company.name = null;
            $scope.checkoutForm.vat.$setPristine();
            return $scope.checkoutForm.companyName.$setPristine();
          }
        });
...
angular.module('gi.security').directive('couponValid', [
  '$q', '$http', 'giCart', function($q, $http, Cart) {
    return {
      restrict: 'A',
      require: 'ngModel',
      compile: function(elem, attrs) {
        var linkFn;
        linkFn = function($scope, elem, attrs, controller) {
          return controller.$asyncValidators.couponValid = function(modelValue, viewValue) {
            var asset, assets, deferred, i, itemParams, len, ref;
            deferred = $q.defer();
            if ((viewValue == null) || viewValue === "") {
              ...
              for (i = 0, len = assets.length; i < len; i++) {
                asset = assets[i];
                if ((ref = asset._data) != null ? ref._id : void 0) {
                  itemParams += "&assets[]=" + asset._data._id;
                }
              }
              $http.get('/api/coupon-info?couponCode=' + viewValue + itemParams).success(function(coupon) {
                if (coupon != null ? coupon.valid : void 0) {
                  Cart.setCoupon(coupon);
                  return deferred.resolve(coupon);
                } else {
                  Cart.setDefaultCoupon();
                  return deferred.reject();
                }
      ...
      compile: function(elem, attrs) {
        ...
        linkFn = function($scope, elem, attrs, controller) {
          return controller.$asyncValidators.giVat = function(modelValue, viewValue) {
            ...
            deferred = $q.defer();
            if ((viewValue == null) || viewValue === "") {
              Cart.calculateTaxRate(viewValue, true).then(function() {
                return deferred.resolve();
              }, function(error) {
                return deferred.resolve();
              });
...
angular.module('gi.commerce').provider('giCart', function() {
  ...
  this.$get = [
    '$q', '$rootScope', '$http', 'giCartItem', 'giLocalStorage', 'giCountry', 'giCurrency', 'giPayment', 'giMarket', 'giUtil', '$window', 'giEcommerceAnalytics', 'giDiscountCode', '$injector', '$timeout', function($q, $rootScope, $http, giCartItem, store, Country, Currency, Payment, Market, Util, $window, giEcommerceAnalytics, Discount, $injector, $timeout) {
      ...
      getSubTotal = function(isTrial, ignoreDiscount) {
        var priceInfo, subTotal;
        subTotal = 0;
        priceInfo = getPricingInfo();
        priceInfo.isTrial = isTrial;
        angular.forEach(cart.items, function(item) {
          return subTotal += item.getSubTotal(priceInfo, ignoreDiscount);
        });
        return +subTotal.toFixed(2);
      };
      getTaxTotal = function(isTrial, ignoreDiscount) {
        var priceInfo, taxSavings, taxTotal;
        taxTotal = 0;
        priceInfo = getPricingInfo();
        priceInfo.isTrial = isTrial;
        angular.forEach(cart.items, function(item) {
          return taxTotal += item.getTaxTotal(priceInfo, ignoreDiscount);
        });
      ...
      calculateTaxRate = function(code, removedVat) {
        ...
        $http.get(uri).success(function(data) {
          ...
          if ((cart.tax > 0) && vatNumber && !removedVat) {
            exp = Util.vatRegex;
      ...
      c = {
        ...
        isLocalBusiness: function() {
          return cart.isLocalBusiness;
        },
        taxName: function() {
          return cart.taxName;
        },
        ...
        isCouponValid: function() {
          return cart.coupon.valid;
        },
        getCouponDuration: function() {
          return cart.coupon.duration;
        },
        getCouponDurationMonths: function() {
          return cart.coupon.duration_in_months;
        },
        getCouponDuration: function() {
          return cart.itemDiscountApplied;
        },
        setItemDiscountApplied: function(applied) {
          return cart.itemDiscountApplied = applied;
        },
        ...
        isCardElementFormValid: function() {
          return cart.cardElementValid;
        },
  ...
  '$rootScope', 'giLocalStorage', function($rootScope, store) {
    ...
    item.prototype.getPrice = function(priceInfo, ignoreDiscount) {
      ...
      if ((((ref = this._priceList) != null ? (ref1 = ref.prices) != null ? ref1[marketCode] : void 0 : void 0) != null) && !(priceInfo.isTrial && this._data.trialItem)) {
        ...
        if (!ignoreDiscount) {
          if (priceInfo.itemDiscountApplied && this._data.itemDiscount) {
            price = price.minus(price.times(this._data.itemDiscount));
          }
        }
        if ((ref2 = priceInfo.coupon) != null ? ref2.valid : void 0) {
    ...
    item.prototype.getSubTotal = function(priceInfo, ignoreDiscount) {
      var itemPrice;
