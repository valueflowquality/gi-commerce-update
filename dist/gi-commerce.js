...
angular.module('gi.commerce').directive('giCheckout', [
  'giCart', 'usSpinnerService', 'Address', 'giPayment', '$modal', 'giUtil', '$q', '$timeout', 'deviceDetector', '$location', 'Auth', '$http', 'giPriceList', function(Cart, Spinner, Address, Payment, $modal, Util, $q, $timeout, deviceDetector, $location, Auth, $http, PriceList) {
    return {
      ...
      link: function($scope, element, attrs) {
        var cardElement, fieldUsed, getErrorMessage, invokeCartPayment, processItemDiscount, purchaseItems, setPaymentDate, stopSpinner, updateCartCountry, wrapSpinner;
        ...
        $scope.$watch('model.me', function(me) {
          if ((me != null ? me.user : void 0) != null) {
            ...
            return $scope.isTrial = !me.user.trialUsed;
          } else {
            Cart.setCustomer();
        ...
        $scope.$watch('model.assets[0]._id', function() {
          var asset, i, item, itemDiscountFound, j, len, len1, ref, ref1;
          itemDiscountFound = false;
        ...
        $scope.getTypeName = function(item) {
          var assetType, i, len, ref, ref1;
          if (item._data.assetTypeId && ((ref = $scope.model) != null ? ref.assetTypes : void 0)) {
            ref1 = $scope.model.assetTypes;
...
$templateCache.put("gi.commerce.summary.html","<a class=\"cart\" href=\"/a/checkout\">\r\n  <img src=\"https://vfq-cdn.s3-eu-west-1.amazonaws.com/public/img/cart.svg\">\r\n  <span> CART ({{ giCart.totalQuantity() }})</span>\r\n</a>\r\n");}]);
    ...
    '$q', '$rootScope', '$http', 'giCartItem', 'giLocalStorage', 'giCountry', 'giCurrency', 'giPayment', 'giMarket', 'giUtil', '$window', 'giEcommerceAnalytics', 'giDiscountCode', '$injector', '$timeout', function($q, $rootScope, $http, giCartItem, store, Country, Currency, Payment, Market, Util, $window, giEcommerceAnalytics, Discount, $injector, $timeout) {
      ...
      c = {
        ...
        submitUserInfo: function() {
          var deferred, registrationObject, that, userRegistrationRequired;
          that = this;
          deferred = $q.defer();
          ...
          if (this.customerInfo && (!this.customer)) {
            ...
            $rootScope.$on('event:accountRequired-failed', function(e, err) {
              $rootScope.$broadcast('giCart:paymentFailed', err);
              return deferred.reject();
            });
            registrationObject = Object.assign({}, this.customerInfo);
            if (this.billingAddress.country) {
              registrationObject.countryCode = this.billingAddress.country;
            }
          ...
          if (cart.client_secret && cart.cardElement) {
            ...
            stripeIns.handleCardPayment(cart.client_secret, cart.cardElement).then(function(result) {
              ...
              if (result.paymentIntent) {
                ...
                }, cart.items, cart.market.code, cart.currency.code);
                ref = cart.items;
                for (i = 0, len = ref.length; i < len; i++) {
                  item = ref[i];
                ...
                return that.waitForAssets(assetIds).then(function() {
                  var assetRedirectInfo, j, len1, ref1;
                  assetRedirectInfo = [];
                  ref1 = cart.items;
                  for (j = 0, len1 = ref1.length; j < len1; j++) {
                    item = ref1[j];
                    assetRedirectInfo.push({
                      redirectUrl: item._data.redirectUrl,
                      redirectPriority: item._data.redirectPriority
                    });
                  }
                  that.empty();
        ...
        redirectUser: function(redirectInfo) {
          var highestPriorityRedirect, i, len, redirect;
          if (!(redirectInfo != null ? redirectInfo.length : void 0) > 0) {
            return $window.location.href = "/welcome";
          } else {
            highestPriorityRedirect = void 0;
            for (i = 0, len = redirectInfo.length; i < len; i++) {
              redirect = redirectInfo[i];
            ...
            if (!highestPriorityRedirect.redirectUrl) {
              ...
              if (highestPriorityRedirect.redirectUrl.match(/^\/(?!\/).*/)) {
                return $window.location.href = highestPriorityRedirect.redirectUrl;
              } else {
                return $window.location.href = "/welcome";
              }
            }
...
angular.module('gi.commerce').factory('giEcommerceAnalytics', [
  'giLog', 'giAnalytics', function(Log, Analytics) {
    ...
    return {
      ...
      sendTransaction: function(obj, items, marketCode, currency) {
        var i, id, itemPrice, j, len, possible, prod, productIds, products, purchase, ref, ref1, rev;
        id = '';
        ...
        if (items != null) {
          for (j = 0, len = items.length; j < len; j++) {
            ...
            itemPrice = 0;
            if (marketCode) {
              itemPrice = (ref = i._priceList) != null ? (ref1 = ref.prices) != null ? ref1[marketCode] : void 0 : void 0;
            }
            prod = {
              id: i._data.id,
              name: i._name,
